...
def Calculate():
    ...
    if (currentfile != ''):
        ...
        if (rez > 0):
            ...
            if (RightEye == False):
                imgLb = cv2.copyMakeBorder(imgL, top=0, bottom=0, left=np.uint16(minDisparities), right=0,
                                           borderType=cv2.BORDER_CONSTANT, value=[155, 155, 155])
            else:
                imgLb = cv2.copyMakeBorder(imgL, top=0, bottom=0, left=0, right=np.uint16(minDisparities),
                                           borderType=cv2.BORDER_CONSTANT, value=[155, 155, 155])
            ...
            if (precisealignmenthack == '0'):
                if (oneeightysetting == '0'):
                    filteredImg = cv2.resize(filteredImg, (width, int(height / 2)),
                                             interpolation=cv2.INTER_CUBIC)  # Disparity truncation hack
                ...
                if (RightEye == False):
                    imgLb2 = cv2.copyMakeBorder(imgL2, top=0, bottom=0, left=np.uint16(minDisparities), right=0,
                                                borderType=cv2.BORDER_CONSTANT, value=[155, 155, 155])
                else:
                    imgLb2 = cv2.copyMakeBorder(imgL2, top=0, bottom=0, left=0, right=np.uint16(minDisparities),
                                                borderType=cv2.BORDER_CONSTANT, value=[155, 155, 155])
    ...
    while True:
        ...
        if (rez > 0):
            ...
            if (stereodepthsetting == '1' and savefile == 1):
                imgLb2 = cv2.copyMakeBorder(imgL, top=0, bottom=0, left=0, right=np.uint16(minDisparities),
                                            borderType=cv2.BORDER_CONSTANT, value=[155, 155, 155])
            ...
            if (precisealignmenthack == '0'):
                ...
                if (stereodepthsetting == '1' and savefile == 1):
                    imgLb2a = cv2.copyMakeBorder(imgL, top=0, bottom=0, left=0, right=np.uint16(minDisparities),
                                                 borderType=cv2.BORDER_CONSTANT, value=[155, 155, 155])
                ...
                if (oneeightysetting == '0'):
                    if (stereodepthsetting == '0' or savefile == 2):
                        filteredImg = cv2.warpAffine(filteredImg, M, (width, int(height / 2)),
                                                     borderMode=cv2.BORDER_WRAP)
                        filteredImg2 = cv2.warpAffine(filteredImg2, M, (width, int(height / 2)),
                                                      borderMode=cv2.BORDER_WRAP)
                    else:
            ...
            if (savefile == 1):
                if (savefiletype == 'JPEG'):
                    cv2.imwrite(batchpathname + '/' + filename + '.jpg', filteredImg,
...
def SaveFile(savefile, batch):
    ...
    if (batch == 0 and savefile != 0 and currentfile != ''):
        ...
        if (savefile == 1):
            if (savefiletype == 'JPEG'):
                cv2.imwrite(pathname + '/' + filename + '_depthmap.jpg', thedepth,
                            [cv2.IMWRITE_JPEG_QUALITY, jpegquality])
    ...
    if (batch == 1 and len(files) >= 1):
        ...
        for i in range(numberofthreads):
            worker = Thread(target=ThreadedCalculate, args=(
            q, w0.get(), w1.get(), w2.get(), w3.get(), w4.get(), w5.get(), w6.get(), w7.get(), savefile))
        ...
        while (index < OutFrame):
            ...
            progressText.config(text='File (' + str(index - InFrame + 1) + '/' + str(
                (OutFrame - InFrame) + 1) + ')   --   ' + '%0.2f' % (100 * (
                        (index - InFrame + 1) / (OutFrame - InFrame + 1))) + '%   --   ' + '%0.2f' % (timeperframe * (
                        (OutFrame - InFrame + 1) - (index - InFrame)) / 60) + ' minutes left')
...
def showadvancedsettings():
    ...
    stereodepthcheck = Checkbutton(advancedsettingsCanvas, variable=stereodepthbool,
                                   command=lambda: advancedsettings(0))
    stereodepthcheck.grid(row=4, column=1, pady=15, columnspan=2)
    Label(advancedsettingsCanvas, text='Precise Alignment Hack\n(doubles processing time)').grid(row=5, column=0,
                                                                                                 padx=5, sticky=E)
...
def saveSettings():
    ...
    settings.update()
    advancedsettings(0)
    settingsstring = str(w0.get()) + ',' + str(w1.get()) + ',' + str(w2.get()) + ',' + str(w3.get()) + ',' + str(
        w4.get()) + ',' + str(w5.get()) + ',' + str(w6.get()) + ',' + str(w7.get()) + ',' + str(
        numberofthreads) + "," + str(savefiletype) + "," + str(jpegquality) + "," + str(oneeightysetting) + "," + str(
        stereodepthsetting) + "," + str(precisealignmenthack)
    try:
        filename = os.path.splitext(os.path.basename(currentfile))[0]
...
stereodepthcheck.grid(row=4, column=1, pady=15, columnspan=2)
Label(advancedsettingsCanvas, text='Precise Alignment Hack\n(doubles processing time)').grid(row=5, column=0, padx=5,
                                                                                             sticky=E)
precisealignmentbool = StringVar(advancedsettingsCanvas)
precisealignmentcheck = Checkbutton(advancedsettingsCanvas, variable=precisealignmentbool)
# precisealignmentcheck.config(width=15)
