...
objp = np.zeros( (CHESSBOARD_SIZE[0]*CHESSBOARD_SIZE[1], 1, 3) , np.float64)
objp[0:,0, :2] = np.mgrid[0:CHESSBOARD_SIZE[0], 0:CHESSBOARD_SIZE[1]].T.reshape(-1, 2)
...
def main():
    ...
    if calib_left != None:
        DIM, K, D = calib_left
        save_left = np.save('Left_calibrated', {'DIM': DIM, 'K': K, 'D': D})
    else:
        print("Error LEFT")
    if calib_right != None:
        DIM, K, D = calib_right
        save_right = np.save('Right_calibrated', {'DIM' : DIM, 'K': K, 'D':D })
    else:
        print('Error RIGHT')

    save = np.save('intrs', {'OBJL': objp_left, 'OBJR' : objp_right, 'IMPL':imgp_left, 'IMGR':imgp_right})
