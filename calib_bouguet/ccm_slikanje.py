...
def take_pictures():
    ...
    dir_left = "/home/minneyar/Desktop/NOVE SLIKE/dir_left"
    dir_right = "/home/minneyar/Desktop/NOVE SLIKE/dir_right"    #if not os.path.exists(directory):
    ...
    while camL.grab() and camR.grab():
        ...
        stackedFrames = np.concatenate((cv2.pyrDown(simplest_cb(RGB_L,1)),cv2.pyrDown(simplest_cb(RGB_R,1))          ), axis=1)

       # cv2.imshow("FRAME LEFT", cv2.pyrDown(simplest_cb(RGB_L,1)))
        #cv2.imshow("FRAME RIGHT", cv2.pyrDown(simplest_cb(RGB_R,1)))
       # cv2.imshow("FRAME RIGHT", cv2.pyrDown(RGB_L))
        cv2.imshow("FRAME RIGHT", stackedFrames)
        ...
        if retL and retR:
            if key == 32:  # Press Space to save img
                ...
                cv2.imwrite(img_l,simplest_cb(RGB_L,1))
                cv2.imwrite(img_r, simplest_cb(RGB_R,1))
